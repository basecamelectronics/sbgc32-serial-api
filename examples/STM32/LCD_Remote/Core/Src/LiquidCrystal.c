...
//GPIO_TypeDef *_port;	// @ Modified @

uint8_t _displayfunction = LCD_4BITMODE | LCD_1LINE | LCD_5x8DOTS; // @ Modified @
			     ...
			     uint16_t d0, uint16_t d1, uint16_t d2, uint16_t d3)
{
  if(_fourbit_mode)
    ...
    init(0, rs, rw, enable, d0, d1, d2, d3, 0, 0, 0, 0);
}
...
//  HAL_GPIO_WritePin(LCD_RS_GPIO_Port, _rs_pin, GPIO_PIN_RESET);
