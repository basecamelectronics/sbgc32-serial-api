...
#define		SBGC_SEVERAL_DEVICES	sbgcON			/*!<  Using more than one controller with the library								*/
...
#define		SBGC_NON_BLOCKING_MODE	sbgcON			/*!<  Provide the library with non-blocking communication							*/
	...
	#define	SBGC_OPTIMIZE_COMMANDS	sbgcON			/*!<  Suppress the new commands if there is the same command in buffer				*/
	#define	SBGC_SEND_IMMEDIATELY	sbgcOFF			/*!<  Send serial commands right away without calling processing functions			*/
	#define	SBGC_MAX_COMMAND_NUM	20				/*!<  1 Min --> 255 Max. Maximum number of serial commands processed simultaneously	*/
#endif
...
#define		SBGC_USE_FREE_RTOS		sbgcON			/*!<  User's applications uses FreeRTOS. Enable support of FreeRTOS					*/
#if (SBGC_USE_AZURE_RTOS || SBGC_USE_FREE_RTOS)
	#define	SBGC_THREAD_STACK_SIZE	256				/*!<  SBGC32 handler thread stack usage volume										*/
	#define	SBGC_THREAD_PRIOR		2				/*!<  SBGC32 handler thread priority												*/
#endif
...
#define		SBGC_USE_STM32_DRIVER	sbgcON			/*!<  Using a STM32 controller as a master device									*/
