...
#define		SBGC_SEVERAL_DEVICES	sbgcON			/*!<  @ref Note1. Using more than one controller with the library					*/
...
#define		SBGC_NON_BLOCKING_MODE	sbgcON			/*!<  Provide the library with non-blocking communication							*/
	...
	#define	SBGC_OPTIMIZE_COMMANDS	sbgcON			/*!<  @ref Note2. Suppress the new commands if there is the same command in buffer 	*/
	...
	#define	SBGC_MAX_COMMAND_NUM	20				/*!<  1 Min --> 255 Max. @ref Note4. Maximum simultaneous serial commands processed	*/
#endif

#define		SBGC_TX_BUFFER_SIZE		3				/*!<  1 Min (256 bytes) --> 8 Max (32768 bytes). Buffer size for sent commands		*/
#define		SBGC_RX_BUFFER_SIZE		3				/*!<  1 Min (256 bytes) --> 8 Max (32768 bytes). Buffer size for received commands	*/
...
#define		SBGC_NEED_DEBUG			sbgcON			/*!<  The system will print debug messages using callback function in driver		*/
	...
	#define	SBGC_LOG_COMMAND_ID		sbgcOFF			/*!<  Add serial command ID to log													*/
	#define	SBGC_LOG_COMMAND_STATUS	sbgcON			/*!<  Add serial command status to log												*/
	#define	SBGC_LOG_COMMAND_PARAM	sbgcOFF			/*!<  Add serial command parameter to log											*/
	#define	SBGC_LOG_COMMAND_DATA	sbgcOFF			/*!<  Add serial command raw payload to log											*/
	#define	SBGC_CUSTOM_SPRINTF		sbgcON			/*!<  Allow to use a custom sprintf function to optimize resources usage. Use the
														  @ref SerialAPI_LinkSprintf function to link a custom sprintf to the library	*/
#endif

#define		SBGC_NEED_ASSERTS		sbgcON			/*!<  Allow commands assert															*/
#define		SBGC_NEED_CONFIRM_CMD	sbgcON			/*!<  Service flag that responsible for CMD_CONFIRM commands handling				*/
#define		SBGC_NEED_REF_INFO		sbgcON			/*!<  Debug mode flag for getting access to auxiliary info about system parameters	*/
...
#define		YAW						2				/*!<  Serial number according to the Euler order. 2 - default value					*/
	...
	#define	SBGC_ADJ_VARS_REF_INFO	sbgcON			/*!<  Debug mode flag for getting access to auxiliary adjustable variable info		*/
	#define	SBGC_ADJ_VARS_NAMES		sbgcON			/*!<  Add names to adjustable variable definition									*/
	#define	SBGC_ADJ_VARS_ADD_FLAG	sbgcON			/*!<  Extend adjustable variables structure by user-defined variables				*/
#endif
	...
	typedef struct
	{
		unsigned char		activeFlag;				/*!<  Customizable parameter of adjustable variable									*/
...
#define		SBGC_USE_FREE_RTOS		sbgcON			/*!<  User's application uses FreeRTOS. Enable support of FreeRTOS					*/
#define		SBGC_USE_PTHREAD_OS		sbgcOFF			/*!<  User's application uses POSIX Thread. Enable support of POSIX Thread			*/
		...
		#define	SBGC_THREAD_PRIOR			2		/*!<  @ref SBGC32_HandlerThread priority											*/
...
#define		SBGC_USE_ESPIDF_DRIVER	sbgcOFF			/*!<  Using an ESP32 controller with ESP-IDF framework as a master device			*/
#define		SBGC_USE_LINUX_DRIVER	sbgcOFF			/*!<  Using a Linux machine as a master device										*/
#define		SBGC_USE_STM32_DRIVER	sbgcON			/*!<  Using an STM32 controller as a master device									*/
	...
	#define	SBGC_DRV_HAL_DMA_UART	sbgcON			/*!<  UART driver works with DMA through HAL										*/
	...
	#define	SBGC_DRV_TX_BUFFER_SIZE	3				/*!<  1 Min (256 bytes) --> 7 Max (16384 bytes). Driver sent data buffer size		*/
	#define	SBGC_DRV_RX_BUFFER_SIZE	3				/*!<  1 Min (256 bytes) --> 7 Max (16384 bytes). Driver received data buffer size	*/
