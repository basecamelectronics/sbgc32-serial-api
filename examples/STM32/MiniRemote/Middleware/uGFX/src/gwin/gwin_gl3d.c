...
#include "board_dep.h"
#include "../../drivers/gdisp/STM32LTDC/stm32_ltdc.h"
...
static void gl3dRedraw(GWindowObject *gh) {
	...
	//memset(zb->pbuf, 0xaa,zb->xsize*zb->ysize*2);

	GDisplay* g = gdispGetDisplay(0);

	blit_area(g, gh->x, gh->y, zb->xsize, zb->ysize, 0, 0, zb->linesize/2/*sizeof(gColor)*/, (const gPixel *)zb->pbuf,
			LTDC_FMT_ARGB8888, DMA2D_INPUT_RGB565, DMA2D_M2M, 0);
}
...
void *pvMalloc( size_t xWantedSize ){
	void* ptr;
	if(xWantedSize > 8192){
		ptr = pvPortMallocExt(xWantedSize);
	}else{
