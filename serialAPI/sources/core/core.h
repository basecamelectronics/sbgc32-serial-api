 ...
 *
 *  @defgroup	Common_Core Common Core
 *  @ingroup	Core
 *  	@brief	Common Core Module
 *
 ...
 * communication driver, do it based on
...
typedef		int						(*SprintfFunc_t)(char* buffer, const char* format, ...);
 ...
 */
#define 	RC_NUM_CHANNELS			5				/*!<  RC_ROLL : RC_PITCH : FC_ROLL : FC_PITCH : RC_YAW								*/
...
#define		RX_NO_TIMEOUT			0U
#define		RX_MAX_TIMEOUT			0xFFFFFFFFU

#define		PR1_START_CHARACTER		0x3EU			/*!<  '>'																			*/
#define		PR2_START_CHARACTER		0x24U			/*!<  '$'																			*/

#define		PR2_CRC16_POLYNOM		0x8005
...
#if (SBGC_REF_INFO)
	#define	PARAM_BLOCK_(name, type)\
									{ name, type, 1 }

	#define	PARAMS_BLOCK_(name, type, len)\
									{ name, type, len }
...
/**	@note @ref SBGC32_ManualInit, 8 arg
 */
typedef enum
{
...
typedef enum
{
	...
	PM_MAIN_PARAMS_EXT_2,							/*!<  See @ref mainParamsExt2_ReferenceInfoArray									*/
	PM_MAIN_PARAMS_EXT_3,							/*!<  See @ref mainParamsExt3_ReferenceInfoArray									*/
 ...
 *			RX 2-5 bytes
 *
 *	@ref	SBGC32_CheckConfirmation function
 */
...
typedef struct
{
	...
	SprintfFunc_t			SprintfFunc;			/*!<  Pointer to sprintf() function. Can be customized due to heavy weight			*/
	...
	/* Private variables */							// 	  Values are not allowed to be manually modified by the user, read only!
	...
	ui16	_rxErrorsCount;							/*!<  Parser error counter when receiving a SerialCommands							*/
		...
		ui8		_unexpectedCommandsBuff
				[UNEXP_CMD_BUFFER_TOTAL_SIZE];		/*!<  Unexpected commands data buffer. Storage format:
	...
	TxRxStatus_t			_parserCurrentStatus;	/*!<  The current state of the input SerialCommand parser							*/
		...
		ConfirmationParams_t
							_confirmationParams;	/*!<  A last received CMD_CONFIRM command											*/

		ConfirmationStatus_t
							_confirmationStatus;	/*!<  System response according CMD_CONFIRM command.
														  Can be useful in a high level software application							*/
...
/**	@note	ConfirmationParams_t.errCode \n
 ...
 *			IMU_ExtCalib_t.IMU_IDx \n
 *			CalibInfo_t.IMU_Type
 */
typedef enum
{
	IMU_TYPE_CURRENTLY_ACTIVE		= 0,			// Only for CMD_SELECT_IMU_3 in extended format. See SBGC32_SelectIMU_3 function
...
typedef struct __PACKED__
{
	...
	/**	@cond
	 */
	extern const ParameterReferenceInfo_t AHRS_DebugInfoReferenceInfoArray [];
...
typedef struct __PACKED__
{
	...
	/**	@cond
	 */
	extern const ParameterReferenceInfo_t motor4_ControlReferenceInfoArray [];
