 ...
 */
	...
	static void UART_DrvInit (Driver_t *driver)
	{
		...
		UART_DrvPinDetermination(driver);
		UART_DrvPeriphDetermination(driver);
		...
		/* UART Init */
			...
			driver->uart->Init.OneBitSampling	= UART_ONE_BIT_SAMPLE_DISABLE;

			driver->uart->AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
			...
			HAL_NVIC_EnableIRQ(driver->UART_IRQn);
				...
				HAL_NVIC_SetPriority(driver->DMA_TxIRQn, 0, 0);
				HAL_NVIC_EnableIRQ(driver->DMA_TxIRQn);

				HAL_NVIC_SetPriority(driver->DMA_RxIRQn, 0, 0);
...
void DriverInit (void *driver, __UART_TYPE_DEF *uart, __TIMER_TYPE_DEF *tim)
{
	...
	#if (DRV_USE_UART_DEBUG) && (DRV_CUSTOM_SETTING)
